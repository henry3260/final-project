...
void linkedList_push(struct music** head, char *new_title, char *new_artist, int new_date[3], float new_length){ // 加到最前 
...
void linkedList_insertAfter(struct music* prev_music, char *new_title, char *new_artist, int new_date[3], float new_length){ // 插入在指定歌曲後
	if (prev_music == NULL) {
        printf("必須有指定歌曲");
        return;
	}
...
void linkedList_append(struct music** head, char *new_title, char *new_artist, int new_date[3], float new_length) { // 加到最尾
	struct music* new_music = (struct music*) malloc(sizeof(struct music));
